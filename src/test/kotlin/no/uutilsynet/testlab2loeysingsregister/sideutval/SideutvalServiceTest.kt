...
@SpringBootTest(
    ...
    properties = ["spring.datasource.url= jdbc:tc:postgresql:16-alpine:///SideutvalServiceTestTest-db"]
)
class SideutvalServiceTest() {

    @Autowired
    lateinit var sideutvalService: SideutvalService

    @MockitoBean
    lateinit var sideutvalRepository: SideutvalRepository
    @MockitoBean
    lateinit var loeysingRepository: LoeysingRepository

    @Test
    fun `getOrCreateSideutval returns list of SideUtvalResponse when loeysing exists and sideutval exists`() {
        val sideutvalLookupRequest = SideutvalLookupRequest("http://test.no", listOf("address1", "address2"))
        ...
        `when`(loeysingRepository.findByUrl(URI("http://test.no").toURL())).thenReturn(listOf(loeysing) as MutableList<LoeysingEntity>?)
